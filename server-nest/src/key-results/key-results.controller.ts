...
export class KeyResultsController {
  ...
  deleteOne(@Param('id') id: string) {
    return this.keyResultService.deleteOne(Number(id));
  }

  @Delete('delete-all/:objective_id')
  deleteMany(@Param('objective_id') objective_id: string) {
    return this.keyResultService.deleteMany(Number(objective_id));
  }

  @Patch('/:id')
  update(@Param('id') id: string, @Body() keyResultDto: UpdateKeyResultDto) {
    return this.keyResultService.update(Number(id), keyResultDto);
  }
  ...
  progress(@Param('keyResultId') keyResultId: string) {
    return this.keyResultService.progress(parseInt(keyResultId));
  }
}
