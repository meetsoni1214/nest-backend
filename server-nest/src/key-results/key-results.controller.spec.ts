...
describe('KeyResultsController', () => {
  ...
  describe('progress()', () => {
    const keyResultId = keyResults[0].id;
    it('should call progress() method of keyResultService', async () => {
      // when
      await controller.progress(keyResultId.toString());
      // then
      expect(service.progress).toHaveBeenCalledWith(keyResultId);
    });
    it('should return progress in percentage', async () => {
      //given
      service.progress.mockResolvedValue({ percentage: 20 });
      //when
      const result = await controller.progress(keyResultId.toString());
      //then
